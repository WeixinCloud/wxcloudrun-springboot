...
public class WxClient {
    ...
    public void prePay(WxPrePayParam param) {
        Map<String, String> map = (Map) JSON.parseObject(JSON.toJSONString(param));
...
//        map.put("openid", param.getOpenid());
        try {
            ...
            String response = HttpRequest.post(WX_CLOUD_PRE_ORDER_URL)
                    ...
                    .body();
            log.info("{},{}", JSON.toJSONString(map), response);
            //String xmlStr = HttpUtils.httpRequest(WX_MCH_PRE_ORDER_URL, "POST", xml);
            // log.info("{},{}", JSON.toJSONString(map), JSON.toJSONString(response));
        } catch (Exception e) {
